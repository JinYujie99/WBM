...
def process_configs(config: Union[CommonArgs, Munch]):
    ...
    model_dirname = f'{config.model.model_name}_{config.model.Katoms}K_{config.model.model_layer}layer_{config.model.dim_hidden}dim_{config.model.dropout_rate}dp_clfnum={config.model.clf_num_hidden}_clfhidden={config.model.clf_hidden_dim}_clfdropout={config.model.clf_dropout}'
    train_dirname = f'{config.train.lr}lr_{config.train.train_bs}bs_{config.train.weight_decay}wd'
