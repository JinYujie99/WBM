    ...
    def forward(self, *args, **kwargs):
        ...
        if self.edge_feat:
            if not self.layered_feature:
                ...
                x, edge_index, edge_attr, batch, batch_size, ptr = self.arguments_read(*args, **kwargs)
                kwargs.pop('batch_size', 'not found')
                out_readout, batch_embedded_features_uncat = self.encoder.forward_JK(x, edge_index, edge_attr, batch, batch_size, ptr **kwargs)
...
class GINEncoder(BasicEncoder):
    ...
    def __init__(self, config: Union[CommonArgs, Munch], *args, **kwargs):
        ...
        if kwargs.get('without_embed'):
            self.conv1 = gnn.GINConv(nn.Sequential(nn.Linear(config.model.dim_hidden, 2 * config.model.dim_hidden, dtype=torch.float64),
                                               nn.BatchNorm1d(2 * config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
                                               nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64)))
        else:
            self.conv1 = gnn.GINConv(nn.Sequential(nn.Linear(config.dataset.dim_node, 2 * config.model.dim_hidden, dtype=torch.float64),
                                               nn.BatchNorm1d(2 * config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
                                               nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64)))

        self.convs = nn.ModuleList(
            [
                gnn.GINConv(nn.Sequential(nn.Linear(config.model.dim_hidden, 2 * config.model.dim_hidden, dtype=torch.float64),
                                      nn.BatchNorm1d(2 * config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
                                      nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64)))
...
class GINMolEncoder(BasicEncoder):
    ...
    def __init__(self, config: Union[CommonArgs, Munch], **kwargs):
        ...
        self.conv1 = GINEConv(nn.Sequential(nn.Linear(config.model.dim_hidden, 2 * config.model.dim_hidden, dtype=torch.float64),
                                            nn.BatchNorm1d(2 * config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
                                            nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64)))

        self.convs = nn.ModuleList(
            [
                GINEConv(nn.Sequential(nn.Linear(config.model.dim_hidden, 2 * config.model.dim_hidden, dtype=torch.float64),
                                       nn.BatchNorm1d(2 * config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
                                       nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64)))
                for _ in range(num_layer - 1)
            ]
    ...
    def get_layered_feature(self, x, edge_index, edge_attr):
        x = self.atom_encoder(x)
        x = x.to(torch.float64)
        layered_features = []
        layered_features.append(x)
    ...
    def forward_JK(self, x, edge_index, edge_attr, batch, batch_size, ptr, **kwargs):
        ...
        batch_embedded_features = torch.cat(layered_features, dim=1)
        batch_embedded_features_uncat = [batch_embedded_features[ptr[k].item(): ptr[k + 1].item(), :] for k in
                                         range(len(ptr) - 1)]
...
class GINEConv(gnn.MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        """"""
        # print(x.shape)  #25*32
        # print(edge_attr.shape)  #25*3
        if self.bone_encoder:
            edge_attr = self.bone_encoder(edge_attr)  #25*3
