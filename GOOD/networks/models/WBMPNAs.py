...
from .PNA import PNAFeatExtractor

@register.model_register
class PNA_FGW(GNNBasic):
    def __init__(self, config: Union[CommonArgs, Munch], deg):
        ...
        self.gnn = PNAFeatExtractor(config, deg=deg)
            ...
            for i, shape in enumerate(unique_atom_shapes):
                ...
                with torch.no_grad():
                    for idx_atom in range(self.Katoms):
                        F = self.Fbar[idx_atom].to(self.device).to(torch.float64)
                        processedC = self.Cbar[idx_atom].to(self.device)
                        edge_index = torch.argwhere(processedC == 1.).T
                        embedded_features = self.gnn.encoder.get_layered_feature(F, edge_index)
    ...
    def batch_to_list(self, data: Batch):
        with torch.no_grad():
            ...
            batch_h = [torch.ones(C.shape[0], dtype=torch.float64, device=self.device) / C.shape[0] for C in batch_C]
