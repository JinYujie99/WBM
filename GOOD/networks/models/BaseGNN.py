...
class GNNBasic(torch.nn.Module):
    ...
    def arguments_read(self, *args, **kwargs):
        ...
        if self.config.model.model_level == 'node':
            ...
            if self.config.model.layered_feature:
                edge_attr = data.edge_attr
                ptr = data.ptr
                return x, edge_index, edge_attr, batch, batch_size, ptr
            else:
                edge_attr = data.edge_attr
        ...
        if self.config.model.layered_feature:
            ptr = data.ptr
            return x, edge_index, batch, batch_size, ptr
...
class BasicEncoder(torch.nn.Module):
    ...
    def __init__(self, config: Union[CommonArgs, Munch], **kwargs):
        ...
        if kwargs.get('no_bn'):
            ...
            self.batch_norm1 = nn.BatchNorm1d(config.model.dim_hidden, dtype=torch.float64)
            self.batch_norms = nn.ModuleList([
                nn.BatchNorm1d(config.model.dim_hidden, dtype=torch.float64)
                for _ in range(num_layer - 1)
            ])
