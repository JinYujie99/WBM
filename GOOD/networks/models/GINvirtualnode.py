...
class VirtualNodeEncoder(torch.nn.Module):
    ...
    def __init__(self, config: Union[CommonArgs, Munch], *args, **kwargs):
        ...
        self.virtual_mlp = nn.Sequential(*(
                [nn.Linear(config.model.dim_hidden, 2 * config.model.dim_hidden, dtype=torch.float64),
                ...
                [nn.Linear(2 * config.model.dim_hidden, config.model.dim_hidden, dtype=torch.float64),
                 nn.BatchNorm1d(config.model.dim_hidden, dtype=torch.float64), nn.ReLU(),
...
class vGINEncoder(GINEncoder, VirtualNodeEncoder):
    ...
    def get_layered_feature(self, x, edge_index):
        self.virtual_mlp.eval()
        x = x.to(torch.float64)
        node_num = x.shape[0]
        batch_size = 1
        batch = torch.zeros(node_num, device=self.config.device, dtype=torch.long)
        layered_features = []
        layered_features.append(x)
        virtual_node_feat = self.virtual_node_embedding(
            torch.zeros(batch_size, device=self.config.device, dtype=torch.long))
