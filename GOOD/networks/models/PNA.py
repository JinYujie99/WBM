...
from .PNA_double import PNAConv
...
class PNA(GNNBasic):
    def __init__(self, config: Union[CommonArgs, Munch]):
        ...
        self.feat_encoder = PNAFeatExtractor(config)
...
class PNAFeatExtractor(GNNBasic):
    def __init__(self, config: Union[CommonArgs, Munch], **kwargs):
        super(PNAFeatExtractor, self).__init__(config)
        num_layer = config.model.model_layer
        self.encoder = PNAEncoder(config, **kwargs)
...
class PNAEncoder(BasicEncoder):
    def __init__(self, config: Union[CommonArgs, Munch], *args, **kwargs):
        super(PNAEncoder, self).__init__(config, *args, **kwargs)
        num_layer = config.model.model_layer
        ...
        if kwargs.get('without_embed'):
            self.conv1 = PNAConv(in_channels=config.model.dim_hidden, out_channels=config.model.dim_hidden,
                                     aggregators=aggregators, scalers=scalers, deg=self.deg)
        else:
            self.conv1 = PNAConv(in_channels=config.dataset.dim_node, out_channels=config.model.dim_hidden,
                                     aggregators=aggregators, scalers=scalers, deg=self.deg)
        self.convs = nn.ModuleList(
            [
                PNAConv(in_channels=config.model.dim_hidden, out_channels=config.model.dim_hidden,
                            aggregators=aggregators, scalers=scalers, deg=self.deg)
