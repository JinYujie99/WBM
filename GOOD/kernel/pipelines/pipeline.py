...
class Pipeline:
    ...
    def __init__(self, task: str, model: torch.nn.Module, loader: Union[DataLoader, Dict[str, DataLoader]],
                 ood_algorithm: BaseOODAlg, config: Union[CommonArgs, Munch]):
    ...
    def train_batch(self, epoch, data: Batch, pbar) -> dict:
        r"""
        ...
        data, targets, mask, node_norm = self.ood_algorithm.input_preprocess(data, targets, mask, node_norm, self.model.training, self.config)

        edge_weight = data.get('edge_norm') if self.config.model.model_level == 'node' else None

        model_output, dist_features = self.model(evaluate=False,  data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm)
        ...
        batch_size = dist_features.shape[0]
        k = int(self.config.model.Katoms / self.config.dataset.num_classes)
        w_loss = 0.0
        for i in range(batch_size):
            local_label = int(data.y[i].item())
            w_loss += torch.mean(dist_features[i, k * local_label:k * (local_label + 1)])
        w_loss /= batch_size
        loss = loss + self.config.ood.ood_param * w_loss
    ...
    def train(self):
        ...
        self.ood_algorithm.set_up(self.model, self.loader, self.config)
        ...
        for epoch in range(self.config.train.ctn_epoch, self.config.train.max_epoch):
            ...
            for index, data in pbar:
                ...
                # train a batch
                train_stat = self.train_batch(epoch, data, pbar)

                mean_loss = (mean_loss * index + self.ood_algorithm.mean_loss.item()) / (index + 1)
            ...
            if (epoch % evaluation_freq == 0):
                ...
                if self.ood_algorithm.spec_loss is not None:
                    ...
                    print(f'#IN#Approximated average training loss {mean_loss:.4f}')


                # epoch_train_stat = self.evaluate('eval_train')  #We do not record train metric/loss for efficiency
                ...
                # checkpoints save
                self.save_epoch(epoch, id_val_stat, id_test_stat, val_stat, test_stat, self.config)
    ...
    def evaluate(self, split: str):
        ...
        for data in pbar:
            ...
            model_output = self.model(evaluate=True, data=data, edge_weight=None, ood_algorithm=self.ood_algorithm)
    ...
    def config_model(self, mode: str):
        r"""
        ...
        if mode == 'test':
            ...
            if os.path.exists(self.config.id_test_ckpt):
                ...
                print(f'#IN#Checkpoint {id_ckpt["epoch"]}: \n-----------------------------------\n'
                      ...
                      f'Train Loss: {id_ckpt["train_loss"]:.4f}\n'
                ...
                print(f'#IN#Checkpoint {ckpt["epoch"]}: \n-----------------------------------\n'
                      ...
                      f'Train Loss: {ckpt["train_loss"]:.4f}\n'
    ...
    def save_epoch(self, epoch: int, id_val_stat: dir, id_test_stat: dir, val_stat: dir, test_stat: dir,
        ...
        """
        full_state_dict = {'gnn_params': self.model.gnn.state_dict(),
                           'classifier_params': self.model.classifier.state_dict(),
                           'atoms_params': [param.clone().detach().cpu() for param in self.model.atoms_params]
                           }
