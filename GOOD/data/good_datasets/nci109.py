...
def load_TUDatasets(dataset_root) -> (Dataset, Dataset, Dataset):
    dataset_path = opj(dataset_root, "NCI109", str(0))
    ret = TUDataset(
    ...
    train, val, test = size_split(ret, opj(dataset_root, "NCI109"))
    return train, val, test

@register.dataset_register
class NCI109(InMemoryDataset):
    ...
    def load(dataset_root: str, domain=None, shift=None, generate=None):
        ...
        return {'train': train_dataset, 'id_val': None, 'id_test': None,
                'val': val_dataset, 'test': test_dataset, 'task': 'Unbalanced Bianry Classification',
